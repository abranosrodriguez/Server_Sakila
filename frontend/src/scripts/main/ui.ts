...
type RowRenderer<T> = (item: T) => string;
type HeaderRenderer<T> = (sampleItem: T) => string;
...
export function loadDataBtn(filmsData: Film[]) {
...
export function loadDataSelect(filmsData: Film[]) {
  removeData();
  createSelect();
  ...
  renderTable<Film>(
    ...
    (film) =>
      `<tr style="text-align:center;">
        ${Object.keys(film)
          .map((k) => `<td>${film[k as keyof Film] ?? ""}</td>`)
          .join("")}
      </tr>`
  );
}
...
export function renderQuery(
  ...
  renderTable<Film>(
    ...
    currentPage,
    pageSize,
    () => `<th>Categoría</th><th>Duración Media</th>`,
    (film) =>
      `<tr>
...
export function renderPagination(
  ...
  for (let i = startPage; i <= endPage; i++) {
    buttons += `<button class="btn ${
      i === currentPage ? "btn-secondary" : "btn-outline-secondary"
